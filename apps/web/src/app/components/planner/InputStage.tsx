    ...
    >
      ...
      >
        {DOMAIN_CHIPS.map(({ label, icon: Icon, color }) => (
          <div
            key={label}
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-xs ${color}`}
      ...
      <div className="relative z-10 w-full max-w-3xl px-6 flex flex-col items-center gap-8">
        ...
        >
          ...
          >
            Enter any research question. Chiron validates novelty against 40M+ papers, then
            generates a complete protocol — materials, budget, timeline, and validation.
          </p>
        </motion.div>
          ...
          <div className="flex flex-col gap-2">
            {EXAMPLE_QUESTIONS.slice(0, 3).map((q, idx) => (
              <motion.button
                ...
                style={{
                  background:
                    activeExample === idx ? "rgba(0,212,255,0.08)" : "rgba(255,255,255,0.02)",
                  border:
                    activeExample === idx
              ...
              >
                <ChevronRight
                  size={14}
                  className="text-cyan-600 mt-0.5 shrink-0 group-hover:text-cyan-400 transition-colors"
                />
                <span
